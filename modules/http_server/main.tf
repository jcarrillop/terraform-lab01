...
resource "google_compute_instance" "http_server-ubuntu" {
  ...
  metadata_startup_script = "sudo apt-get update && sudo apt-get install apache2 -y && echo '<html><body><h1>Environment: ${local.network} - '${HOSTNAME}' - </h1></body></html>' | sudo tee /var/www/html/index.html"
  ...
  network_interface {
    subnetwork = "${var.subnet}-subnet-02"
